	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if( path == null || path.equals( "" ) || path.equals( "list" ) ) {
				...
				url = "/mariadb/model2/board_list1.jsp";
			} else if( path.equals( "view" ) ) {
				action = new ViewAction();
				action.execute(request, response);
				
				url = "/mariadb/model2/board_view1.jsp";
			} else if( path.equals( "write" ) ) {
				action = new WriteAction();
				action.execute(request, response);
				
				url = "/mariadb/model2/board_write1.jsp";
			} else if( path.equals( "write_ok" ) ) {
				action = new WriteOkAction();
				action.execute(request, response);
				
				url = "/mariadb/model2/board_write1_ok.jsp";
			} else if( path.equals( "modify" ) ) {
				action = new ModifyAction();
				action.execute(request, response);
				
				url = "/mariadb/model2/board_modify2.jsp";
			} else if( path.equals( "modify_ok" ) ) {
				action = new ModifyOkAction();
				action.execute(request, response);
				
				url = "/mariadb/model2/board_modify1_ok.jsp";
			} else if( path.equals( "delete" ) ) {
				action = new DeleteAction();
				action.execute(request, response);
				
				url = "/mariadb/model2/board_delete1.jsp";
			} else if( path.equals( "delete_ok" ) ) {
				action = new DeleteOkAction();
				action.execute(request, response);
				
				url = "/mariadb/model2/board_delete1_ok.jsp";
			}
