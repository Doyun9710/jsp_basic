...
<% 
	...
	int totalRecord = 0;	// 총 글 개수, select count(*) from emot_board1;
	int recordPerPage = 10;	// 페이지 당 글 개수
	int totalPage = 1;		// 총 페이지 수
	int blockPerPage = 5;	// 페이지 묶음
	...
	try {
		...
		String sql = "select seq, emot, subject, writer, date_format(wdate, '%Y-%m-%d') wdate, hit, datediff(now(), wdate) wgap from emot_board1 order by seq desc";
...
<%
	...
	for( int i=startBlock ; i<=endBlock ; i++ ) {
		if(i==cpage) out.println( "<span><a href='board_list1.jsp?cpage=" + i + "'>[" + i + "]</a></span>" );
		else out.println( "<span><a href='board_list1.jsp?cpage=" + i + "'>" + i + "</a></span>" );
	}
